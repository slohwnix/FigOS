use crate::assets::{
    FONT, PSF1_MAGIC, PSF2_MAGIC,
    Psf1Header, Psf2Header,
};
...
impl Framebuffer {
    ...
    pub unsafe fn scroll(
        ...
        if scroll_height == 0 || scroll_height >= self.height {
            return;
        }
        ...
        match bg_color {
            None => {
                ...
                core::ptr::write_bytes(clear_start, 0u8, pixels_to_clear);
            }
            Some(color) => {
    ...
    pub unsafe fn draw_char(&self, c: char, x: usize, y: usize, color: u32) {
        ...
        if FONT.starts_with(&PSF1_MAGIC) {
            ...
            let glyph_size = header.char_size as usize;
            let glyph_ptr = FONT.as_ptr().add(
                header.header_size as usize + glyph_index * glyph_size
            );
                ...
                for b in 0..bytes_per_line {
                    ...
                    for col in 0..8 {
                        ...
                        if px < header.width as usize {
                            if (byte & 0x80) != 0 {
                                self.draw_pixel(x + px, y + row, color);
                            }
                            byte <<= 1;
                        }
                    }
